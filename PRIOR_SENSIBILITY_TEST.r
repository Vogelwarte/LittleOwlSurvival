...
beta.win <- runif(10000,-2, 2)          # Prior for winter weather effect on survival probability on logit scale
...
beta.mass <- rnorm(10000,0, precconv(1))            # Prior for body mass effect on survival probability on logit scale
beta.feed <- rnorm(10000,0, precconv(1))            # Prior for supplementary feeding effect on survival probability on logit scale
...
INPUT<-data.frame(lp.mean,beta.feed,beta.win,beta.mass,beta.male,surv.raneff, simul=1)
FAKEDATA<-expand.grid(sex=c(0,1),mass=c(0),feed=0,win=scale(c(0,4,8,12))) %>% mutate(simul=1)
...
FAKEDATA %>% full_join(INPUT, by='simul',relationship = "many-to-many") %>%
  mutate(logit_phi=
           beta.mass*mass +       ### survival varies by body mass
