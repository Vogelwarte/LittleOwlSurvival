...
mean.phi <- rbeta(10000,90, 25)   # uninformative prior for all biweekly survival probabilities
...
beta.win <- rnorm(10000,0, precconv(1))          # Prior for winter weather effect on survival probability on logit scale
...
sigma.surv <- runif(10000,0, 1)                     # Prior for standard deviation of survival for random individual effect
...
FAKEDATA<-expand.grid(sex=c(0,1),age=c(0),win=c(0,3,6,9)) %>% mutate(simul=1)
