...
## UPDATED ON 23 NOV TO INCLUDE DEAD RECOVERY AND MULTIPLE STATES - MISMATCHING ID's between data files makes this impossible!!!
...
dat2009<-read_excel("data/EH Master_juveniles_Moggi.xlsx",sheet="EH 2009 daily") %>%
  filter(Adult==0) %>%
  select(-Adult,-`Remove after last capture`,-`Hatching 2`,-Remarks) %>%
  rename(LastAliveDate=`Last known to be alive`,DeathDate=`Known to be dead`,Ring=`Ring Number`) %>%
  gather(key="Date",value="Status",-Individual, -Sex,-Treatment,-Hatching,-LastAliveDate,-DeathDate,-Ring) %>%
  ...
  group_by(Individual, Sex,Treatment,HatchOCC,OCC,DeathDate,Ring) %>%
...
keepinds<- bind_rows(dat2009,dat2010,dat2011) %>%
  ...
  group_by(Individual, Sex,Treatment,HatchOCC,DeathDate,Ring) %>%
