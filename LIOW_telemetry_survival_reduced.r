...
AnnTab<-crossing(data.frame(season=c(1,2,3,3,3,3,4),
                   ...
                   snow=c(0,0,0,4,8,12,0)),
                   feeding=c(0,1),
                   scaleweight=c(-2,0,2),  ## summary(weight_scale)
                   sex=c(0,1)) %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
### get the median duration of PF from our sample of birds
median(apply(CH[,which(season==1)],1,sum))
...
mild.season.surv<-MCMCpred %>% rename(raw.surv=surv) %>%
  ...
  group_by(season,Season,sex,scaleweight,feeding) %>%
  summarise(surv=quantile(raw.surv,0.5),surv.lcl=quantile(raw.surv,0.025),surv.ucl=quantile(raw.surv,0.975)) %>%
  ungroup() %>%
...
harsh.winter.surv<-MCMCpred %>% rename(raw.surv=surv) %>%
  ...
  group_by(season,Season,snow,sex,scaleweight,feeding) %>%
  summarise(surv=quantile(raw.surv,0.5),surv.lcl=quantile(raw.surv,0.025),surv.ucl=quantile(raw.surv,0.975)) %>%
  ungroup() %>%
...
TableS2<- mild.season.surv %>%
  ...
  mutate(Mass=ifelse(scaleweight<0,"light",ifelse(scaleweight==0,"average","heavy"))) %>%
  arrange(Supplemented,Sex,scaleweight,season) %>%
  select(Supplemented,Sex,Mass,Season,scaleweight,season,mild.survival)
...
TableS2<- harsh.winter.surv %>%
  ...
  mutate(Supplemented=ifelse(feeding==1,"Yes","No")) %>%
  mutate(Mass=ifelse(scaleweight<0,"light",ifelse(scaleweight==0,"average","heavy"))) %>%
  arrange(Supplemented,Sex,scaleweight) %>%
  ungroup() %>%
  select(Supplemented,Sex,Mass,Season,scaleweight,season,harsh.survival) %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ##########  ABANDONED CODE TO TROUBLESHOOT SURVIVAL ESTIMATE MISMATCH #################################
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

### commented out on 24 NOV 2023 after understanding why extrapolated annual survival is extremely dependent on length of post-fledging interval
...
#   mutate(season=rep(c(rep(1,4),rep(2,6),rep(3,10),rep(4,6)),each=3)) %>%
...
#       z.rep[i,t] ~ dbern(phi[i,t-1] * z.rep[i,t-1]) # replicate z (true state)
#       # Observation process
#       y[i,t] ~ dbern(p[i,t] * z[i,t]*nulleff.mat[i,t])
#       y.rep[i,t] ~ dbern(p[i,t] * z.rep[i,t]*nulleff.mat[i,t]) # replicate y (observations)
# 
#     
#     } #t end
#       #Derived parameters
